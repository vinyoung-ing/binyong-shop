...
- **가격표 통째로 붙여넣기**: 카테고리를 고른 뒤 이 버튼을 누르고 가격표 텍스트를 붙여넣으면
  그룹(챕터1, 올클…)과 항목(샤면 0.05…)이 한 번에 등록됩니다. 등록 전에 미리보기로 확인할 수 있어요.
...
```
index.html          홈 (home/ 에서 빌드된 결과물 — 직접 수정하지 말 것)
assets/             홈 빌드 결과물 (React + Three.js 번들)
home/               홈 원본 소스 (React + React Three Fiber)
...
홈만 React + Three.js로 되어 있고, 나머지 페이지는 빌드 없이 그대로 동작합니다.
`home/` 안의 파일을 고친 경우에만 다시 빌드해서 결과물을 함께 커밋하세요.

```bash
cd home
npm install      # 처음 한 번만
npm run build    # 루트의 index.html 과 assets/ 를 새로 만듭니다
```

`js/` 공용 모듈(헤더/푸터, 가격표 불러오기 등)은 홈 번들에 복사되지 않고 실행 시점에 불러오므로,
`js/` 쪽만 고쳤다면 다시 빌드할 필요가 없습니다.
